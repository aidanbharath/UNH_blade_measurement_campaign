...
def main():
    ...
    start, stop = '2022-11-22 18:45:40', '2022-11-22 20:00:10'
    ...
    #for df in pwr.columns:

    pwr = pwr.iloc[4:]

    print(pwr.max(), pwr.min())
    print(pwr.mean(), pwr.std())
    ...
    ax1_5, = ax1.plot(rtime,s2.values[:,0],label='SG-5',alpha=0.85,linestyle=':')
    ax1_6, = ax1.plot(rtime,s2.values[:,1],label='SG-6',alpha=0.85,linestyle=':')
    ax1_7, = ax1.plot(rtime,s2.values[:,2],label='SG-7',alpha=0.85,linestyle=':')
    ax1_8, = ax1.plot(rtime,s2.values[:,3],label='SG-8',alpha=0.85,linestyle=':')
